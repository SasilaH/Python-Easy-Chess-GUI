...
logging.basicConfig(
	filename="pecg_log.txt", filemode="w", level=logging.DEBUG, format=log_format
)
...
ico_path = {
	"win32": {
		"pecg": "Icon/pecg.ico",
		"enemy": "Icon/enemy.ico",
		"adviser": "Icon/adviser.ico",
	},
	"linux": {
		"pecg": "Icon/pecg.png",
		"enemy": "Icon/enemy.png",
...
class Timer:
	def __init__(self, tc_type="fischer", base=300000, inc=10000, period_moves=40):
		"""
...
# def aIsendToArduino(ai_move, is_kill, piece_type):
	...
	if (is_kill):
		piece_name = "kill"
		toPiece = [ord(ai_move.uci()[2]) - ord(prev_move[2]), int(ai_move.uci()[3]) - int(prev_move[3])]
		send_string = piece_name + ";" + str(toPiece[0]) + ";" + str(toPiece[1]) + ";" + 'h1' + ";" + 'h1'
		print(send_string)
...
class RunEngine(threading.Thread):
					...
					for n in p["options"]:
						...
						if user_value != default_value:
							try:
								...
								logging.info(
									"Set {} to {}".format(n["name"], user_value)
								)
							except Exception:
	...
	def run(self):
		...
		if self.analysis:
			...
			with self.engine.analysis(self.board, limit) as analysis:
				for info in analysis:
					...
					try:
						...
						if "score" in info:
							self.score = (
								int(info["score"].relative.score(mate_score=32000))
								/ 100
							)
						...
						if (
							not is_time_check
							and self.tc_type != "fischer"
							and self.tc_type != "delay"
							and time.perf_counter() - start_time >= self.base_ms / 1000
						):
			...
			try:
				self.score = (
					int(result.info["score"].relative.score(mate_score=32000)) / 100
				)
			except KeyError:
				self.score = 0
...
class EasyChessGui:
		...
		try:
			if sys_os == "Windows":
				engine = chess.engine.SimpleEngine.popen_uci(
					path_and_file, cwd=folder, creationflags=subprocess.CREATE_NO_WINDOW
				)
			else:
			...
			for p in data:
				if p["name"] == eng_id_name:
					...
					try:
						...
						logging.info(
							"This engine {} has no options.".format(eng_id_name)
						)
						break
					except Exception:
		...
		for opt in opt_dict:
			...
			if o.type == "spin":
				...
				if o.name.lower() == "threads":
					value = 1
					logging.info("config {} is set to {}".format(o.name, value))
	...
	def update_text_box(self, window, msg, is_hide):
		...
		if not "bestmove " in msg_str:
			...
			try:
				...
				sg.Popup(
					"Engine error, it sent a {} bestmove.\n".format(best_move)
					+ "Back to Neutral mode, it is better to "
					"change engine {}.".format(self.opp_id_name),
					icon=ico_path[platform]["pecg"],
					title=BOX_TITLE,
				)
	...
	def change_square_color(self, window, row, col):
		...
		bd_sq_color = (
			self.move_sq_dark_color if is_dark_square else self.move_sq_light_color
		)
	...
	def select_promotion_piece(self, stm):
		...
		psg_promote_board = (
			copy.deepcopy(white_init_promote_board)
			if stm
	...
	def get_promo_piece(self, move, stm, human):
		...
		if human:
			...
			if psg_promo is None:
				logging.info(
					"User did not select a promotion piece, " "set this to queen."
				)
	...
	def set_depth_limit(self):
		...
		user_depth = sg.PopupGetText(
			"Current depth is {}\n\nInput depth [{} to {}]".format(
				self.max_depth, MIN_DEPTH, MAX_DEPTH
			),
			title=BOX_TITLE,
			icon=ico_path[platform]["pecg"],
		)
		...
		while not board.is_game_over(claim_draw=True):
			...
			if is_human_stm:
				...
				while True:
					...
					# Mode: Play, stm: User
					if (
						button == "User Wins::user_wins_k"
						or is_search_stop_for_user_wins
					):
					...
					# Mode: Play, stm: User
					if (
						button == "User Draws::user_draws_k"
						or is_search_stop_for_user_draws
					):
					...
					if type(button) is tuple:
						...
						if move_state == 0:
							move_from = button
							fr_row, fr_col = move_from
							piece = self.psg_board[fr_row][
								fr_col
							]  # get the move-from piece
						...
						# Else if to_sq button is pressed
							...
							if (
								self.relative_row(to_sq, board.turn) == RANK_8
								and moved_piece == chess.PAWN
							):
								is_promote = True
								...
								user_move = chess.Move(
									fr_sq, to_sq, promotion=pyc_promo
								)
							else:
							...
							if user_move in board.legal_moves:
								...
								self.update_game(
									move_cnt, user_move, time_left, user_comment
								)
			...
			# Else if side to move is not human
				...
				if best_move is None:
					...
					while True:
						...
						if button == "Show::right_book1_k":
							is_hide_book1 = False
							ref_book1 = GuiBook(
								self.computer_book_file, board, self.is_random_book
							)
							all_moves, is_found = ref_book1.get_all_moves()
							if is_found:
								window.Element("polyglot_book1_k").Update(all_moves)
						...
						if button == "Show::right_book2_k":
							is_hide_book2 = False
							ref_book2 = GuiBook(
								self.human_book_file, board, self.is_random_book
							)
							all_moves, is_found = ref_book2.get_all_moves()
							if is_found:
								window.Element("polyglot_book2_k").Update(all_moves)
						...
						best_move = self.update_text_box(
							window, msg, is_hide_search_info
						)
				...
				window.find_element("_gamestatus_").Update("Mode     Play")
				sendToArduino(best_move, is_kill, board.piece_type_at(to_sq))
	...
	def main_loop(self):
		...
		while True:
				...
				while True:
					...
					if e == "Delete Player":
						try:
							...
							logging.info(e)
							sg.Popup(
								"Please locate your pgn file by "
			...
			if button == "User::tc_k":
				...
				layout = [
					[
						...
						sg.Input(
							self.human_base_time_ms / 60 / 1000,
							key="base_time_k",
							size=(8, 1),
						),
					],
					[
						...
						sg.Input(
							self.human_inc_time_ms / 1000, key="inc_time_k", size=(8, 1)
						),
					],
					[
			...
			if button == "Engine::tc_k":
				...
				layout = [
					[
						sg.T("Base time (minute)", size=(16, 1)),
						sg.Input(
							self.engine_base_time_ms / 60 / 1000,
							key="base_time_k",
							size=(8, 1),
						),
					],
					...
					[
						...
						sg.Radio(
							...
							key="timepermove_k",
							default=True
							if self.engine_tc_type == "timepermove"
							else False,
							tooltip="Only base time will be used.",
						),
					],
					[sg.OK(), sg.Cancel()],
				]
			...
			if button == "Set Name::user_name_k":
				...
				layout = [
					[sg.Text("Current username: {}".format(self.username))],
					[
						sg.T("Name", size=(4, 1)),
						sg.Input(self.username, key="username_k", size=(32, 1)),
					],
					[sg.OK(), sg.Cancel()],
				]
				window.Disable()
						...
						while True:
							...
							if e1 == "OK":
								try:
									...
									logging.exception(
										"Failed to get engine " "path and file"
									)
			...
			if button == "Edit":
				...
				window.Disable()
				edit_win = sg.Window(
					button_title, layout=edit_layout, icon=ico_path[platform]["pecg"]
				)
				...
				while True:
					...
					if e == "Modify":
						...
						try:
							...
							sg.Popup(
								"Please select an engine to modify.",
								title="/Edit/Modify",
								icon=ico_path[platform]["pecg"],
							)
							continue
						...
						option_layout.append(
							[
								...
								sg.Input(
									engine_id_name, size=(38, 1), key="string_name_k"
								),
							]
						)
				...
				if not is_cancel_edit_win and not is_cancel_modify_win:
					self.update_engine_to_config_file(
						engine_path_file, engine_id_name, orig_idname, ret_opt_name
					)
					self.engine_id_name_list = self.get_engine_id_name_list()
			...
			if button == "Delete":
				...
				delete_win = sg.Window(
					button_title, layout=delete_layout, icon=ico_path[platform]["pecg"]
				)
				is_cancel = False
				while True:
					...
					if e == "Delete":
						try:
							...
							sg.Popup(
								"Please select an engine to delete.",
								title=button_title,
								icon=ico_path[platform]["pecg"],
							)
							continue
						...
						for i in range(len(data)):
							if data[i]["name"] == engine_id_name:
								logging.info(
									"{} is found for deletion.".format(engine_id_name)
								)
								data.pop(i)
								break
			...
			if button == "Set Engine Opponent":
				...
				layout = [
					[
						sg.T(
							"Current Opponent: {}".format(self.opp_id_name),
							size=(40, 1),
						)
					],
					[
					...
					if e == "OK":
						...
						try:
							...
							(
								self.opp_file,
								self.opp_path_and_file,
							) = self.get_engine_file(engine_id_name)

						except IndexError:
							logging.info(
								"User presses OK but did not select " "an engine."
							)
						except Exception:
			...
			if button == "Set Engine Adviser":
				...
				layout = [
					[
						sg.T(
							"Current Adviser: {}".format(self.adviser_id_name),
							size=(40, 1),
						)
					],
					[
					...
					[
						...
						sg.Spin(
							[t for t in range(1, 3600, 1)],
							initial_value=self.adviser_movetime_sec,
							size=(8, 1),
							key="adviser_movetime_k",
						),
					],
					[sg.OK(), sg.Cancel()],
				]
				...
				while True:
					...
					if e == "OK":
						...
						try:
							adviser_eng_id_name = self.adviser_id_name = v[
								"adviser_id_name_k"
							][0]
							(
								self.adviser_file,
								self.adviser_path_and_file,
							) = self.get_engine_file(adviser_eng_id_name)
						except IndexError:
			...
			if button == "Set Book::book_set_k":
				...
				w = sg.Window(
					BOX_TITLE + "/Set Book", layout, icon=ico_path[platform]["pecg"]
				)
				window.Disable()
			...
			if button == "Game::settings_game_k":
				...
				layout = [
					[
						sg.CBox(
							"Save time left in game notation",
			...
			if button == "Play":
				if engine_id_name is None:
					...
					sg.Popup(
						"Install engine first! in Engine/Manage/Install",
						icon=ico_path[platform]["pecg"],
						title="Mode",
					)
					continue
...
def main():
	...
	pecg = EasyChessGui(
		...
		user_config_file,
		pecg_book,
		book_from_computer_games,
		book_from_human_games,
		is_use_gui_book,
		is_random_book,
		max_book_ply,
	)
